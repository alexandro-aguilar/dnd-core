...
import { inputGetItemSchema } from './inputGetItemSchema';
...
export const handler = middy(
  async (event: APIGatewayProxyEventV2, context: Context): Promise<APIGatewayProxyResultV2<Array<ItemDto>>> => {
    logger.addContext({ requestId: context.awsRequestId });

    const getItemsController = container.get<GetItemsController>(types.GetItemsController);
    const response = await getItemsController.execute();

    return response;
  }
)
  .use(requestValidator(inputGetItemSchema))
