...
import { inputGetAbilitySchema } from './inputGetAbilitySchema';
...
import GetAbilitiesController from '../controllers/GetAbilitiesController';
...
export const handler = middy(
  async (event: APIGatewayProxyEventV2, context: Context): Promise<APIGatewayProxyResultV2<Array<AbilityDto>>> => {
    logger.addContext({ requestId: context.awsRequestId });
    logger.info('env:', { env: process.env });

    const getAbilitiesController = container.get<GetAbilitiesController>(types.GetAbilitiesController);
    const response = await getAbilitiesController.execute();

    return response;
  }
)
  .use(requestValidator(inputGetAbilitySchema))
