...
import PostSessionController from '../controllers/PostSessionController';
import { inputPostSessionSchema } from './inputPostSessionSchema';
...
export const handler = middy(
  async (event: ParsedAPIGatewayProxyEventV2, context: Context): Promise<APIGatewayProxyResultV2<SessionDto>> => {
    logger.addContext({ requestId: context.awsRequestId });

    const postSessionController = container.get<PostSessionController>(types.PostSessionController);
    const response = await postSessionController.execute(event);

    return response;
  }
)
  .use(requestValidator(inputPostSessionSchema))
