...
export const handler = middy(
  async (event: APIGatewayProxyEventV2, context: Context): Promise<APIGatewayProxyResultV2<Array<UserDto>>> => {
    logger.addContext({ requestId: context.awsRequestId });

    const getUsersController = container.get<GetUsersController>(types.GetUsersController);
